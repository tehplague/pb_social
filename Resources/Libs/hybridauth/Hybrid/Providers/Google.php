...
class Hybrid_Providers_Google extends Hybrid_Provider_Model_OAuth2
{
	...
	*/
     ...
     * @param string $type
     */
    function getUserActivity($type = "public")
    {
        // refresh tokens if needed
        $this->refreshToken();
        $activities = array();

        $response = $this->api->api("https://www.googleapis.com/plus/v1/people/me/activities/".$type."");
        if( !$response ){ return ARRAY(); }

        foreach( $response->items as $item ){
            $ua = new Hybrid_User_Activity();
            $ua->id = $item->actor->id;
            $ua->date = $item->updated;
            $ua->text = $item->object->attachments[0]->displayName;
            $ua->raw = $item;
            $activities[] = $ua;
        }
